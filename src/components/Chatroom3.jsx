...
const AdminChat3 = () => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const response = await axios.get("https://back-endt.onrender.com/messages3");
  ...
  const handleSendMessage = async () => {
    if (inputValue.trim() !== "") {
      try {
        await axios.post("https://back-endt.onrender.com/messages3", {
          user_role: "admin",
          message: inputValue,
        });
  ...
  const handleResponse = async (id, response) => {
    try {
      await axios.put(`https://back-endt.onrender.com/messages3/${id}`, { response });
      setMessages(
        messages.map((msg) =>
